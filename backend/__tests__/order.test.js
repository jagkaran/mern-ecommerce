...
beforeAll(async () => {
  ...
  const product = await Product.create({
    name: "Order Product", description: "desc", price: 100,
...
describe("Order API — authenticated user", () => {
  it("GET /api/v1/orders/me → 200 with auth", async () => {
...
describe("Order API — admin", () => {
  it("GET /api/v1/admin/orders → 200 with admin auth", async () => {
    if (!adminCookie) return;
    const res = await request(app).get("/api/v1/admin/orders").set("Cookie", adminCookie);
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body.orders)).toBe(true);
  });
  it("DELETE /api/v1/admin/order/:id → 200 (first delete)", async () => {
