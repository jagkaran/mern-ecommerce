...
const allDeps = Object.assign({}, pkgRaw.dependencies, pkgRaw.devDependencies);
const missing = REQUIRED_PACKAGES.filter((p) => !allDeps[p]);
if (missing.length > 0) {
  ...
  console.log("   ok  All required security packages already present.");
}
...
try {
  ...
  try {
    const audit = JSON.parse(e.stdout ? e.stdout.toString() : "{}");
    const hi = (audit.metadata && audit.metadata.vulnerabilities && audit.metadata.vulnerabilities.high)   || 0;
    const cr = (audit.metadata && audit.metadata.vulnerabilities && audit.metadata.vulnerabilities.critical) || 0;
    if (cr > 0) fail("npm audit: " + cr + " critical vulnerabilities");
    if (hi > 0) warn("npm audit: " + hi + " high vulnerabilities");
  } catch (_) { warn("npm audit returned non-zero"); }
}
...
if (fs.existsSync(appFile)) {
  ...
  if (!src.includes("helmet"))        fail("Missing helmet middleware");
  if (!src.includes("rateLimit"))     fail("Missing express-rate-limit");
  if (!src.includes("mongoSanitize")) fail("Missing express-mongo-sanitize");
  if (!src.includes("xss"))          fail("Missing xss-clean middleware");
  if (!src.includes("cors"))         fail("Missing cors middleware");
  if (!src.includes("compression"))  warn("Missing compression middleware");
}
...
// 4. Secret pattern scan -- skip test files and files using env vars
